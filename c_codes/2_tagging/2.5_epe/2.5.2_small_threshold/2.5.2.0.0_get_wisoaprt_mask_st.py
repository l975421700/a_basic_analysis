...
# -----------------------------------------------------------------------------
# region get wisoaprt_epe_st

wisoaprt_epe_st = {}
wisoaprt_epe_st[expid[i]] = {}
wisoaprt_epe_st[expid[i]]['quantiles'] = {}
wisoaprt_epe_st[expid[i]]['mask'] = {}
wisoaprt_epe_st[expid[i]]['masked_data'] = {}

# set a threshold of 0.002 mm/d
wisoaprt_epe_st[expid[i]]['masked_data']['original'] = \
...
for iqtl in quantiles.keys():
    ...
    #-------- calculate quantiles
    wisoaprt_epe_st[expid[i]]['quantiles'][iqtl] = \
        wisoaprt_epe_st[expid[i]]['masked_data']['original'].quantile(
    ...
    wisoaprt_epe_st[expid[i]]['mask'][iqtl] = \
        (wisoaprt_alltime[expid[i]]['daily'].sel(wisotype=1).copy() >= \
            wisoaprt_epe_st[expid[i]]['quantiles'][iqtl]).compute()
...
with open(
    ...
    'wb') as f:
    pickle.dump(wisoaprt_epe_st[expid[i]], f)
...
res001 = wisoaprt_epe_st[expid[i]]['masked_data']['original'][:, ilat, ilon]
res002 = wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
    wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon] >= (0.002 / seconds_per_d),
    other=np.nan,)
...
for iqtl in quantiles.keys():
    ...
    res01 = wisoaprt_epe_st[expid[i]]['quantiles'][iqtl][ilat, ilon].values
    res02 = np.nanquantile(res002, quantiles[iqtl],)
    print(res01 == res02)
    #-------- check ['mask'][iqtl]
    res11 = wisoaprt_epe_st[expid[i]]['mask'][iqtl][:, ilat, ilon]
...
asizeof.asizeof(wisoaprt_epe_st) / 2**30
asizeof.asizeof(wisoaprt_epe_st[expid[i]]['quantiles']) / 2**30
asizeof.asizeof(wisoaprt_epe_st[expid[i]]['mask']) / 2**30
