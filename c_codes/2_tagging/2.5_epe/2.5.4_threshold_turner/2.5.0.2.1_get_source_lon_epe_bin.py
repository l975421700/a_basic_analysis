...
for iqtl in epe_weighted_sinlon.keys():
    epe_weighted_lon[iqtl] = {}
    
    for ialltime in epe_weighted_sinlon[iqtl].keys():
        print(iqtl + ' - ' + ialltime)
        
        epe_weighted_lon[iqtl][ialltime] = sincoslon_2_lon(
            epe_weighted_sinlon[iqtl][ialltime],
...
for iqtl in epe_weighted_sinlon.keys():
    ...
    for ialltime in epe_weighted_sinlon[iqtl].keys():
        ...
        if (ialltime != 'am'):
            # ialltime = 'mon'
            sinlon = epe_weighted_sinlon[iqtl][ialltime][itime, ilat, ilon]
            coslon = epe_weighted_coslon[iqtl][ialltime][itime, ilat, ilon]
            lon1 = epe_weighted_lon[iqtl][ialltime][itime, ilat, ilon]
            lon2 = np.arctan2(sinlon, coslon) * 180 / np.pi
            if (lon2 < 0): lon2 += 360
            print(((lon1 - lon2) / lon1).values)
        else:
            sinlon = epe_weighted_sinlon[iqtl][ialltime][ilat, ilon]
            coslon = epe_weighted_coslon[iqtl][ialltime][ilat, ilon]
            lon1 = epe_weighted_lon[iqtl][ialltime][ilat, ilon]
            lon2 = np.arctan2(sinlon, coslon) * 180 / np.pi
            if (lon2 < 0): lon2 += 360
            print(((lon1 - lon2) / lon1).values)
