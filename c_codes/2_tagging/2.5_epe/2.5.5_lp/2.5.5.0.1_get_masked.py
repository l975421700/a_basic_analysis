...
# quantile_interval  = np.arange(1, 99 + 1e-4, 1, dtype=np.int64)
quantile_interval  = np.arange(10, 90 + 1e-4, 10, dtype=np.int64)
quantiles = dict(zip(
    [str(x) + '%' for x in quantile_interval],
    [x/100 for x in quantile_interval],
    ))
...
for iqtl in ['10%', '50%']:
    ...
    masked_data = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
            wisoaprt_cum_qtl[expid[i]]['mask'][iqtl][:, ilat, ilon],
            other=0,
        ).compute()
    masked_data_nan = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
    ...
    for ialltime in ['mon', 'sea', 'ann', 'mm', 'sm']:
        ...
        data2 = wisoaprt_masked_lp[expid[i]]['mean'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        ...
        data2 = wisoaprt_masked_lp[expid[i]]['frc'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
        ...
        data2 = wisoaprt_masked_lp[expid[i]]['meannan'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
