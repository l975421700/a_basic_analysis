...
wisoaprt_masked_nt = {}
wisoaprt_masked_nt[expid[i]] = {}
wisoaprt_masked_nt[expid[i]]['mean'] = {}
wisoaprt_masked_nt[expid[i]]['frc']  = {}
wisoaprt_masked_nt[expid[i]]['meannan'] = {}
...
for iqtl in quantiles.keys():
    ...
    wisoaprt_masked_nt[expid[i]]['mean'][iqtl] = mon_sea_ann(masked_data)
    wisoaprt_masked_nt[expid[i]]['mean'][iqtl].pop('daily')
    # wisoaprt_masked_nt[expid[i]]['mean'][iqtl].pop('mon')
    
    wisoaprt_masked_nt[expid[i]]['frc'][iqtl] = {}
    
    for ialltime in wisoaprt_masked_nt[expid[i]]['mean'][iqtl].keys():
        wisoaprt_masked_nt[expid[i]]['frc'][iqtl][ialltime] = \
            (wisoaprt_masked_nt[expid[i]]['mean'][iqtl][ialltime] / \
    ...
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl] = {}
    # am
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['am'] = masked_data_nan.mean(
        dim='time', skipna=True).compute()
    # sm
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['sm'] = masked_data_nan.groupby(
        'time.season').mean(skipna=True).compute()
    # mm
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['mm'] = masked_data_nan.groupby(
        'time.month').mean(skipna=True).compute()
    # ann
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['ann'] = masked_data_nan.resample({'time': '1Y'}).mean(skipna=True).compute()
    # sea
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['sea'] = masked_data_nan.resample({'time': 'Q-FEB'}).mean(skipna=True)[1:-1].compute()
    # mon
    wisoaprt_masked_nt[expid[i]]['meannan'][iqtl]['mon'] = masked_data_nan.resample({'time': '1M'}).mean(skipna=True).compute()

with open(
    exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.wisoaprt_masked_nt.pkl',
    'wb') as f:
    pickle.dump(wisoaprt_masked_nt[expid[i]], f)
...
for iqtl in quantiles.keys():
    ...
    masked_data = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
            wisoaprt_epe_nt[expid[i]]['mask'][iqtl][:, ilat, ilon],
            other=0,
        ).compute()
    masked_data_nan = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
    ...
    with open(
        exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.wisoaprt_masked_nt.pkl',
        'rb') as f:
        wisoaprt_masked_nt[expid[i]] = pickle.load(f)
    
    for ialltime in ['mon', 'sea', 'ann', 'mm', 'sm']:
        # ialltime = 'mon'
        # check mean_values
        data1 = mean_values[ialltime].values
        data2 = wisoaprt_masked_nt[expid[i]]['mean'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        ...
        data2 = wisoaprt_masked_nt[expid[i]]['frc'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
        ...
        data2 = wisoaprt_masked_nt[expid[i]]['meannan'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
