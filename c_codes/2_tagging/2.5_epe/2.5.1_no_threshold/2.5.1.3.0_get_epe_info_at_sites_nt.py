...
# -----------------------------------------------------------------------------
# region get wisoaprt_masked_bin_nt at ice core sites

wisoaprt_masked_bin_nt = {}
with open(
    exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.wisoaprt_masked_bin_nt.pkl',
    'rb') as f:
    wisoaprt_masked_bin_nt[expid[i]] = pickle.load(f)
...
wisoaprt_masked_bin_nt_icores[expid[i]] = {}
...
for icores in stations_sites.Site:
    ...
    wisoaprt_masked_bin_nt_icores[expid[i]][icores] = {}
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'] = {}
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'] = {}
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'] = {}
    
    for ialltime in ['mon', 'sea', 'ann', 'mm', 'sm']:
        # ialltime = 'mon'
        print('#---- ' + ialltime)
        
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'][ialltime] = {}
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'][ialltime] = {}
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime] = {}
        
        for iqtl in quantiles_bin.keys():
            # iqtl = '90%'
            # print('#---- ' + iqtl)
            
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'][ialltime][iqtl] = \
                wisoaprt_masked_bin_nt[expid[i]]['mean'][iqtl][ialltime][
                    :,
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']]
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'][ialltime][iqtl] = \
                wisoaprt_masked_bin_nt[expid[i]]['frc'][iqtl][ialltime][
                    :,
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']]
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime][iqtl] = \
                wisoaprt_masked_bin_nt[expid[i]]['meannan'][iqtl][ialltime][
                    :,
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']]
    ...
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'][ialltime] = pd.DataFrame(
        columns=('iqtl', 'quantiles', 'am',))
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'][ialltime] = pd.DataFrame(
        columns=('iqtl', 'quantiles', 'am',))
    wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime] = pd.DataFrame(
        columns=('iqtl', 'quantiles', 'am',))
    ...
    for iqtl in quantiles_bin.keys():
        ...
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'][ialltime] = pd.concat([
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['mean'][ialltime],
            pd.DataFrame(data={
                'iqtl': iqtl,
                'quantiles': quantiles_bin[iqtl],
                'am': wisoaprt_masked_bin_nt[expid[i]]['mean'][iqtl][ialltime][
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']].values,
                }, index=[0])],
            ignore_index=True,)
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'][ialltime] = pd.concat([
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['frc'][ialltime],
            pd.DataFrame(data={
                'iqtl': iqtl,
                'quantiles': quantiles_bin[iqtl],
                'am': wisoaprt_masked_bin_nt[expid[i]]['frc'][iqtl][ialltime][
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']].values,
                }, index=[0])],
            ignore_index=True,)
        wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime] = pd.concat([
            wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime],
            pd.DataFrame(data={
                'iqtl': iqtl,
                'quantiles': quantiles_bin[iqtl],
                'am': wisoaprt_masked_bin_nt[expid[i]]['meannan'][iqtl][ialltime][
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']].values,
                }, index=[0])],
            ignore_index=True,)
...
with open(
    ...
    'wb') as f:
    pickle.dump(wisoaprt_masked_bin_nt_icores[expid[i]], f)
...
for icores in stations_sites.Site:
    ...
    glat = wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan']['ann'][iqtl].lat.values
    glon = wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan']['ann'][iqtl].lon.values
...
for icores in stations_sites.Site:
    ...
    for ialltime in ['mon', 'sea', 'ann', 'mm', 'sm']:
        ...
        for iqtl in quantiles_bin.keys():
            ...
            d1=wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime][iqtl]
            d2=wisoaprt_masked_bin_nt[expid[i]]['meannan'][iqtl][ialltime][
                    :,
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']]
    ...
    for ialltime in ['am']:
        ...
        for iqtl in quantiles_bin.keys():
            ...
            d1=wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime][
                wisoaprt_masked_bin_nt_icores[expid[i]][icores]['meannan'][ialltime].iqtl == iqtl
            ].am.values
            d2=wisoaprt_masked_bin_nt[expid[i]]['meannan'][iqtl][ialltime][
                    t63_sites_indices[icores]['ilat'],
                    t63_sites_indices[icores]['ilon']].values
...
# -----------------------------------------------------------------------------
# region get wisoaprt_mask_bin_nt at ice core sites
...
wisoaprt_mask_bin_nt_icores[expid[i]] = {}

for icores in stations_sites.Site:
    # icores = 'EDC'
    print('#-------- ' + icores)
    
    wisoaprt_mask_bin_nt_icores[expid[i]][icores] = {}
    
    wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'] = {}
    
    for iqtl in quantiles_bin.keys():
        # iqtl = '90.5%'
        # print('#---- ' + iqtl)
        
        wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'][iqtl] = \
            wisoaprt_mask_bin_nt[iqtl][
                :,
                t63_sites_indices[icores]['ilat'],
                t63_sites_indices[icores]['ilon']]
    
    wisoaprt_mask_bin_nt_icores[expid[i]][icores]['am'] = pd.DataFrame(
        columns=('iqtl', 'quantiles', 'am',))
    ...
    for iqtl in quantiles_bin.keys():
        ...
        wisoaprt_mask_bin_nt_icores[expid[i]][icores]['am'] = pd.concat([
            wisoaprt_mask_bin_nt_icores[expid[i]][icores]['am'],
            pd.DataFrame(data={
                'iqtl': iqtl,
                'quantiles': quantiles_bin[iqtl],
                'am': wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'][
...
with open(
    ...
    'wb') as f:
    pickle.dump(wisoaprt_mask_bin_nt_icores[expid[i]], f)
...
for icores in stations_sites.Site:
    ...
    slat = t63_sites_indices[icores]['lat']
    slon = t63_sites_indices[icores]['lon']
    glat = wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'][iqtl].lat.values
    glon = wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'][iqtl].lon.values
...
iqtl = '90.5%'
icores = 'EDC'
wisoaprt_mask_bin_nt_icores[expid[i]][icores]['daily'][iqtl].mean() * 365
wisoaprt_mask_bin_nt_icores[expid[i]][icores]['am'].am.values
