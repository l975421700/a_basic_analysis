...
# -----------------------------------------------------------------------------
# region import pre_weighted_lon
...
for i in range(len(expid)):
    # i = 0
    with open(exp_odir[i] + expid[i] + '/analysis/echam/' + expid[i] + '.pre_weighted_lon.pkl', 'rb') as f:
        pre_weighted_lon[expid[i]] = pickle.load(f)

    print(exp_odir[i] + '    ' + expid[i])

lon = pre_weighted_lon[expid[0]]['am'].lon
lat = pre_weighted_lon[expid[0]]['am'].lat
...
# -----------------------------------------------------------------------------
# region plot am_sm pre_weighted_lon


#-------- plot configuration
output_png = 'figures/6_awi/6.1_echam6/6.1.3_source_var/6.1.3.1_lon/' + '6.1.3.1 pi_lig_lgm ' + expid[0] + ' ' + expid[1] + ' ' + expid[2] + ' pre_weighted_lon am_sm Antarctica.png'
...
for i in range(len(expid)):
    ...
    #     pre_weighted_lon[expid[i]]['ann'].std(
...
# -----------------------------------------------------------------------------
# region plot am_sm diff pre_weighted_lon


#-------- plot configuration
output_png = 'figures/6_awi/6.1_echam6/6.1.3_source_var/6.1.3.1_lon/' + '6.1.3.1 pi_lig_lgm ' + expid[0] + ' ' + expid[1] + ' ' + expid[2] + ' pre_weighted_lon am_sm diff Antarctica.png'
cbar_label1 = 'Precipitation-weighted open-oceanic relative source longitude [$°$]'
...
for i in np.arange(1, len(expid)):
    ...
    plt_mesh2 = axs[i, 0].pcolormesh(
        lon, lat,
        calc_lon_diff(pre_weighted_lon[expid[i]]['am'],
                      pre_weighted_lon[expid[0]]['am']),
        norm=pltnorm2, cmap=pltcmp2, transform=ccrs.PlateCarree(),)
    
    wwtest_res = circ.watson_williams(
        pre_weighted_lon[expid[i]]['ann'].values * np.pi / 180,
        pre_weighted_lon[expid[0]]['ann'].values * np.pi / 180,
        axis=0,
    )[0] < 0.05
    axs[i, 0].scatter(
    ...
    for iseason in range(len(seasons)):
        ...
        axs[i, iseason + 1].pcolormesh(
            ...
            calc_lon_diff(
                pre_weighted_lon[expid[i]]['sm'].sel(season=seasons[iseason]),
                pre_weighted_lon[expid[0]]['sm'].sel(season=seasons[iseason])
            ),
            norm=pltnorm2, cmap=pltcmp2,transform=ccrs.PlateCarree(),)
        wwtest_res = circ.watson_williams(
            pre_weighted_lon[expid[i]]['sea'].sel(
            ...
            pre_weighted_lon[expid[0]]['sea'].sel(
                time=(pre_weighted_lon[expid[0]]['sea'].time.dt.month == \
                seasons_last_num[iseason])).values * np.pi / 180,
            axis=0,
        )[0] < 0.05
