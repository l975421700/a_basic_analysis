...
pltlevel, pltticks, pltnorm, pltcmp = plt_mesh_pars(
    cm_min=74, cm_max=82, cm_interval1=1, cm_interval2=1, cmap='cividis',
    reversed=False)
...
#-------- Am
plt_data = pre_weighted_rh2m[expid[i]]['am'].copy()
...
plt_data = pre_weighted_rh2m[expid[i]]['ann'].std(
...
for iseason in range(len(seasons)):
    plt_data = pre_weighted_rh2m[expid[i]]['sm'].sel(season=seasons[iseason]).copy()
    # plt_data.values[echam6_t63_ais_mask['mask']['AIS'] == False] = np.nan
    ...
    plt_data = pre_weighted_rh2m[expid[i]]['sea'].sel(
...
# -----------------------------------------------------------------------------
# region check pre_weighted_rh2m at EAIS/WAIS/AP
...
for imask in ['AIS', 'EAIS', 'WAIS', 'AP']:
    ...
    source_rh2m_max = np.max(pre_weighted_rh2m[expid[i]]['am'].values[mask])
    source_rh2m_min = np.min(pre_weighted_rh2m[expid[i]]['am'].values[mask])
    print(np.round(source_rh2m_max, 1))
    print(np.round(source_rh2m_min, 1))
...
for imask in ['AIS', 'EAIS', 'WAIS', 'AP']:
    ...
    source_rh2m = np.average(
        pre_weighted_rh2m[expid[i]]['am'].values[mask],
        weights = echam6_t63_cellarea.cell_area.values[mask] * \
            ocean_aprt_alltime[expid[i]]['am'].sel(var_names='rh2m').values[
                mask],
    )
    
    print(np.round(source_rh2m, 1))
...
imask = 'AIS'
mask = echam6_t63_ais_mask['mask'][imask]
djf_jja_rh2m = (pre_weighted_rh2m[expid[i]]['sm'].sel(season = 'DJF') - \
    pre_weighted_rh2m[expid[i]]['sm'].sel(season = 'JJA')).compute()

np.average(
    djf_jja_rh2m.values[mask],
    weights = echam6_t63_cellarea.cell_area.values[mask],
)
np.mean(djf_jja_rh2m.values[mask])
