...
# -----------------------------------------------------------------------------
# region plot pre_weighted_wind10 am + am aprt
...
pltlevel, pltticks, pltnorm, pltcmp = plt_mesh_pars(
    cm_min=9.8, cm_max=11.4, cm_interval1=0.2, cm_interval2=0.2, cmap='PiYG',)
...
cbar.ax.set_xlabel('Source wind10 [$m \; s^{-1}$]', linespacing=2)
...
pltlevel, pltticks, pltnorm, pltcmp = plt_mesh_pars(
    cm_min=8.5, cm_max=12.5, cm_interval1=0.5, cm_interval2=0.5, cmap='PiYG',)

ctr_level = np.array([0.5, 1, 1.5, ])
...
plt_data = pre_weighted_wind10[expid[i]]['ann'].std(
...
for iseason in range(len(seasons)):
    plt_data = pre_weighted_wind10[expid[i]]['sm'].sel(season=seasons[iseason])
    plt_data.values[echam6_t63_ais_mask['mask']['AIS'] == False] = np.nan
    ...
    plt_data = pre_weighted_wind10[expid[i]]['sea'].sel(
...
pltlevel, pltticks, pltnorm, pltcmp = plt_mesh_pars(
    cm_min=-3, cm_max=-1, cm_interval1=0.25, cm_interval2=0.25, cmap='PiYG',)
...
# -----------------------------------------------------------------------------
# region cross check pre_weighted_wind10 am

#-------- import data
output_png = 'figures/6_awi/6.1_echam6/6.1.3_source_var/6.1.3.4_wind10/6.1.3.4 ' + expid[i] + ' pre_weighted_wind10 am cross_check.png'
file_dir = 'output/echam-6.3.05p2-wiso/pi/'
pre_weighted_var_files = [
    'pi_m_405_4.7/analysis/echam/pi_m_405_4.7.pre_weighted_wind10_am.nc',
    'pi_m_409_4.8/analysis/echam/pi_m_409_4.8.pre_weighted_wind10_am.nc',
]
...
pre_weighted_wind10 = {}
with open(exp_odir + expid[i] + '/analysis/echam/source_var_short/' + expid[i] + '.pre_weighted_wind10.pkl', 'rb') as f:
    pre_weighted_wind10[expid[i]] = pickle.load(f)

#-------------------------------- framework to plot
pltlevel, pltticks, pltnorm, pltcmp = plt_mesh_pars(
    cm_min=4, cm_max=14, cm_interval1=1, cm_interval2=2, cmap='PRGn',)
pltlevel2, pltticks2, pltnorm2, pltcmp2 = plt_mesh_pars(
    cm_min=-0.25, cm_max=0.25, cm_interval1=0.05, cm_interval2=0.1,cmap='BrBG',)

cbar_label1 = 'Source wind10 [$m \; s^{-1}$]'
cbar_label2 = 'Diff. in source wind10 [$m \; s^{-1}$]'
...
# plot am values
plt_mesh1 = axs[0].pcolormesh(
    lon, lat, pre_weighted_wind10[expid[i]]['am'],
    norm=pltnorm, cmap=pltcmp,transform=ccrs.PlateCarree(),)
# plot am norm - lowres
plt_mesh2 = axs[1].pcolormesh(
    lon, lat, pre_weighted_var['am_lowres'].pre_weighted_wind10_am - \
        pre_weighted_wind10[expid[i]]['am'],
    norm=pltnorm2, cmap=pltcmp2,transform=ccrs.PlateCarree(),)
# plot am norm - highres
plt_mesh2 = axs[2].pcolormesh(
    lon, lat, pre_weighted_var['am_highres'].pre_weighted_wind10_am - \
        pre_weighted_wind10[expid[i]]['am'],
    norm=pltnorm2, cmap=pltcmp2,transform=ccrs.PlateCarree(),)
...
plt.text(
    0.5, 1.05, 'Binning (1 $m \; s^{-1}$ wind10 bins) vs scaling approach',
    transform=axs[1].transAxes, ha='center', va='center', rotation='horizontal')
plt.text(
    0.5, 1.05, 'Binning (0.5 $m \; s^{-1}$ wind10 bins) vs scaling approach',
...
# -----------------------------------------------------------------------------
# region check pre_weighted_wind10 at EAIS/WAIS/AP
...
for imask in ['AIS', 'EAIS', 'WAIS', 'AP']:
    ...
    source_wind10_max = np.max(pre_weighted_wind10[expid[i]]['am'].values[mask])
    source_wind10_min = np.min(pre_weighted_wind10[expid[i]]['am'].values[mask])
    print(np.round(source_wind10_max, 1))
    print(np.round(source_wind10_min, 1))
...
for imask in ['AIS', 'EAIS', 'WAIS', 'AP']:
    ...
    source_wind10 = np.average(
        pre_weighted_wind10[expid[i]]['am'].values[mask],
        weights = echam6_t63_cellarea.cell_area.values[mask] * \
            ocean_aprt_alltime[expid[i]]['am'].sel(var_names='wind10').values[
                mask],
    )
    
    print(np.round(source_wind10, 1))
...
imask = 'AIS'
mask = echam6_t63_ais_mask['mask'][imask]
djf_jja_wind10 = (pre_weighted_wind10[expid[i]]['sm'].sel(season = 'DJF') - \
    pre_weighted_wind10[expid[i]]['sm'].sel(season = 'JJA')).compute()

np.average(
    djf_jja_wind10.values[mask],
    weights = echam6_t63_cellarea.cell_area.values[mask],
)
np.mean(djf_jja_wind10.values[mask])
