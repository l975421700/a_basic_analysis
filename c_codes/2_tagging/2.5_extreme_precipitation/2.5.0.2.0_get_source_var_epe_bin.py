...
for ivar in np.arange(2, 6, 1):
    ...
    for iqtl in ['90.5%']: # quantiles_bin.keys():
        ...
        #---- check
        print((epe_var_scaled_pre[iqtl].values[wisoaprt_mask_bin[iqtl]] == var_scaled_pre.values[wisoaprt_mask_bin[iqtl]]).all())
        print((epe_var_scaled_pre[iqtl].values[wisoaprt_mask_bin[iqtl] == False] == 0).all())
        print((epe_ocean_pre[iqtl].values[wisoaprt_mask_bin[iqtl]] == ocean_pre.values[wisoaprt_mask_bin[iqtl]]).all())
        print((epe_ocean_pre[iqtl].values[wisoaprt_mask_bin[iqtl] == False] == 0).all())
    ...
    del epe_weighted_var, ocean_pre, var_scaled_pre, epe_var_scaled_pre, epe_ocean_pre, epe_var_scaled_pre_alltime, epe_ocean_pre_alltime, res01, res02
