...
transport_distance_epe_st[expid[i]] = {}

begin_time = datetime.datetime.now()
print(begin_time)

for iqtl in epe_st_weighted_lon[expid[i]].keys(): # ['90%', '95%']:
    transport_distance_epe_st[expid[i]][iqtl] = {}
    
    for ialltime in ['ann', 'am']:
        print(iqtl + ' - ' + ialltime)
        transport_distance_epe_st[expid[i]][iqtl][ialltime] = \
            epe_st_weighted_lat[expid[i]][iqtl][ialltime].copy().rename(
                'transport_distance_epe_st')
        transport_distance_epe_st[expid[i]][iqtl][ialltime][:] = 0
        
        if (ialltime in ['mon', 'sea', 'ann']):
            print(ialltime)
            
            years = np.unique(
                transport_distance_epe_st[expid[i]][iqtl][ialltime].time.dt.year)
            ...
            for iyear in years:
                ...
                lon_src_flatten = epe_st_weighted_lon[expid[i]][iqtl][ialltime][
                    time_indices].values.reshape(-1, 1).copy()
                lat_src_flatten = epe_st_weighted_lat[expid[i]][iqtl][ialltime][
                    time_indices].values.reshape(-1, 1).copy()
                source_pairs = [[x, y] for x, y in \
                    zip(lat_src_flatten, lon_src_flatten)]
                
                transport_distance_epe_st[expid[i]][iqtl][ialltime][time_indices] = haversine_vector(
                    local_pairs, source_pairs, normalize=True).reshape(
                        transport_distance_epe_st[expid[i]][iqtl][ialltime][time_indices].shape)
                print(datetime.datetime.now() - begin_time)
        elif (ialltime in ['mm', 'sm', 'am']):
            print(ialltime)
            b_lon_2d = np.broadcast_to(
                lon_2d, epe_st_weighted_lon[expid[i]][iqtl][ialltime].shape, )
            b_lat_2d = np.broadcast_to(
                lat_2d, epe_st_weighted_lat[expid[i]][iqtl][ialltime].shape, )
            b_lon_2d_flatten = b_lon_2d.reshape(-1, 1)
            b_lat_2d_flatten = b_lat_2d.reshape(-1, 1)
            local_pairs = [[x, y] for x, y in \
                zip(b_lat_2d_flatten, b_lon_2d_flatten)]
            
            lon_src_flatten = epe_st_weighted_lon[expid[i]][iqtl][
                ialltime].values.reshape(-1, 1).copy()
            lat_src_flatten = epe_st_weighted_lat[expid[i]][iqtl][
                ialltime].values.reshape(-1, 1).copy()
            source_pairs = [[x, y] for x, y in \
                zip(lat_src_flatten, lon_src_flatten)]
            
            transport_distance_epe_st[expid[i]][iqtl][ialltime][:] = haversine_vector(
                local_pairs, source_pairs, normalize=True).reshape(
                    transport_distance_epe_st[expid[i]][iqtl][ialltime].shape)

with open(exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.transport_distance_epe_st_binned.pkl', 'wb') as f:
    pickle.dump(transport_distance_epe_st[expid[i]], f)
