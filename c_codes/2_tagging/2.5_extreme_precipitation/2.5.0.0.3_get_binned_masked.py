...
wisoaprt_masked_bin = {}
wisoaprt_masked_bin[expid[i]] = {}
wisoaprt_masked_bin[expid[i]]['mean'] = {}
wisoaprt_masked_bin[expid[i]]['frc']  = {}
wisoaprt_masked_bin[expid[i]]['meannan'] = {}
...
for iqtl in quantiles_bin.keys():
    ...
    wisoaprt_masked_bin[expid[i]]['mean'][iqtl] = mon_sea_ann(masked_data)
    wisoaprt_masked_bin[expid[i]]['mean'][iqtl].pop('daily')
    # wisoaprt_masked_bin[expid[i]]['mean'][iqtl].pop('mon')
    
    wisoaprt_masked_bin[expid[i]]['frc'][iqtl] = {}
    
    for ialltime in wisoaprt_masked_bin[expid[i]]['mean'][iqtl].keys():
    ...
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl] = {}
    # am
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['am'] = masked_data_nan.mean(
        dim='time', skipna=True).compute()
    # sm
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['sm'] = masked_data_nan.groupby(
        'time.season').mean(skipna=True).compute()
    # mm
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['mm'] = masked_data_nan.groupby(
        'time.month').mean(skipna=True).compute()
    # ann
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['ann'] = masked_data_nan.resample({'time': '1Y'}).mean(skipna=True).compute()
    # sea
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['sea'] = masked_data_nan.resample({'time': 'Q-FEB'}).mean(skipna=True)[1:-1].compute()
    # mon
    wisoaprt_masked_bin[expid[i]]['meannan'][iqtl]['mon'] = masked_data_nan.resample({'time': '1M'}).mean(skipna=True).compute()

with open(
    exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.wisoaprt_masked_bin.pkl',
    'wb') as f:
    pickle.dump(wisoaprt_masked_bin[expid[i]], f)
...
for iqtl in quantiles_bin.keys():
    ...
    masked_data = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
            wisoaprt_mask_bin[iqtl][:, ilat, ilon],
            other=0,
        ).compute()
    masked_data_nan = \
        wisoaprt_alltime[expid[i]]['daily'][:, 0, ilat, ilon].copy().where(
    ...
    for ialltime in ['mon', 'sea', 'ann', 'mm', 'sm']:
        ...
        data2 = wisoaprt_masked_bin[expid[i]]['mean'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        ...
        data2 = wisoaprt_masked_bin[expid[i]]['frc'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
        ...
        data2 = wisoaprt_masked_bin[expid[i]]['meannan'][iqtl][ialltime].values[
            :, ilat, ilon
        ]
        # print((data1[np.isfinite(data1)] == data2[np.isfinite(data2)]).all())
        print(np.max(data1[np.isfinite(data1)] - data2[np.isfinite(data2)]))
