...
sincoslon_scaled_pre_sea[expid[i]] = sincoslon_scaled_pre[expid[i]][12:36].groupby('time.season').sum(dim="time", skipna=True)
...
sincoslon_scaled_pre_am[expid[i]] = sincoslon_scaled_pre[expid[i]][12:36].mean(dim="time", skipna=True)
...
np.arcsin(aaa) * 180 / np.pi
np.arccos(bbb) * 180 / np.pi
