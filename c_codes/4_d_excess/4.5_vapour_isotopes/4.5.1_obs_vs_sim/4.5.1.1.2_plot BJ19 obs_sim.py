...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    subset = np.isfinite(BJ19_polarstern_1d_sim[expid[i]][var_name]) & np.isfinite(BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim'])
    
    print(np.round(pearsonr(BJ19_polarstern_1d_sim[expid[i]][var_name][subset], BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim'][subset], ).statistic ** 2, 3))
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_SLM.png'
    
    fig, ax = plt.subplots(1, 1, figsize=np.array([8.8, 8.8]) / 2.54)
    
    xdata = BJ19_polarstern_1d_sim[expid[i]][var_name].copy()
    ydata = BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim'].copy()
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_SIC.png'
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_altitude.png'
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_altitude SLM0 SIC0.png'
    ...
    ydata = BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim'].copy()
    # subset = (np.isfinite(xdata) & np.isfinite(ydata))
    subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0))
    xdata = xdata[subset]
    ydata = ydata[subset]
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_q.png'
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_q SLM0 SIC0.png'
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_q SLM0 SIC0 lat_0_60.png'
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_q SLM0 SIC0 lat_20_60.png'
    ...
    # subset = (np.isfinite(xdata) & np.isfinite(ydata))
    # subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0))
    # subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] >= -60) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] <= 0))
    subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] >= -60) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] <= -20))
    xdata = xdata[subset]
    ydata = ydata[subset]
    ...
    cdata = BJ19_polarstern_1d_sim[expid[i]]['q'].copy() * 1000
    cdata = cdata[subset]
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_oo2q.png'
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_oo2q SLM0 SIC0.png'
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_oo2q SLM0 SIC0 lat_20_60.png'
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_lat.png'
    # output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_lat SLM0 SIC0.png'
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 observed vs. simulated daily ' + var_name + ' colored_by_lat SLM0 SIC0 lat_20_60.png'
    ...
    cdata = abs(BJ19_polarstern_1d_sim[expid[i]]['lat'].copy())
    cdata = cdata[subset]
...
xdata = BJ19_polarstern_1d_sim[expid[i]]['lon']
ydata = BJ19_polarstern_1d_sim[expid[i]]['lat']

subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] >= -60) & (BJ19_polarstern_1d_sim[expid[i]]['lat'] <= -20))
xdata = xdata[subset]
ydata = ydata[subset]

output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.4 BJ19 daily observation locations SLM0 SIC0 lat_20_60.png'

# output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.4 BJ19 daily observation locations.png'
...
fig, ax = hemisphere_conic_plot(
    lat_min=-85, lat_max=-20, lon_min=-90, lon_max=30,
    add_grid_labels=True, lat_min_tick = -80, lon_min_tick = -90,)
ax.scatter(
    xdata,
    ydata,
    marker='x', s=12, lw=1,
...
for icount, var_name in enumerate(['dD', 'd18O', 'd_xs', 'd_ln', 'q']):
    ...
    lat_subset = BJ19_polarstern_1d_sim[expid[i]]['lat'][subset]
    lon_subset = BJ19_polarstern_1d_sim[expid[i]]['lon'][subset]
    var_subset = BJ19_polarstern_1d_sim[expid[i]][var_name][subset]
    var_sim_subset = BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim'][subset]
    var_diff_subset = (var_sim_subset - var_subset)
    # print(stats.describe(var_subset, axis=None))
    
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.1 ' + expid[i] + ' maps of BJ19 observed vs. simulated daily ' + var_name + '.png'
...
xdata = BJ19_polarstern_1d_sim[expid[i]]['lat']
ydata = BJ19_1d_oo2q

subset = (np.isfinite(xdata) & np.isfinite(ydata) & (BJ19_1d_SLM == 0) & (BJ19_1d_SIC == 0))
output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.0 ' + expid[i] + ' BJ19 daily oo2q vs. latitude SLM0 SIC0.png'

fig, ax = plt.subplots(1, 1, figsize=np.array([8.8, 8.8]) / 2.54)
ax.scatter(xdata[subset], ydata[subset], marker='x', lw=0.5, )

ax.set_xticks(np.arange(-60, 60+1e-4, 30))
ax.xaxis.set_minor_locator(AutoMinorLocator(2))
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.2_BJ19/8.3.0.2.2 ' + expid[i] + ' BJ19 diff. in observed and simulated daily ' + var_name + ' against latitude.png'
    
    fig, ax = plt.subplots(1, 1, figsize=np.array([6.6, 6.6]) / 2.54)
    
    var_obs = BJ19_polarstern_1d_sim[expid[i]][var_name]
    var_sim = BJ19_polarstern_1d_sim[expid[i]][var_name + '_sim']
    subset = (np.isfinite(var_obs) & np.isfinite(var_sim))
    var_obs = var_obs[subset]
    var_sim = var_sim[subset]
    var_lat = BJ19_polarstern_1d_sim[expid[i]]['lat'][subset]
