...
CB19_DDU_1d_sim = {}
with open(exp_odir + expid[i] + '/analysis/echam/' + expid[i] + '.CB19_DDU_1d_sim.pkl', 'rb') as f:
    CB19_DDU_1d_sim[expid[i]] = pickle.load(f)
...
CB19_1d_oo2q = find_multi_gridvalue_at_site_time(
    CB19_DDU_1d_sim[expid[i]]['time'],
    CB19_DDU_1d_sim[expid[i]]['lat'],
    CB19_DDU_1d_sim[expid[i]]['lon'],
...
'''
# Over ocean in ECHAM6
T63GR15_jan_surf = xr.open_dataset('albedo_scratch/output/echam-6.3.05p2-wiso/pi/nudged_701_5.0/input/echam/unit.24')
CB19_1d_SLM = find_gridvalue_at_site(
    CB19_DDU_1d_sim[expid[i]]['lat'].values[0],
    CB19_DDU_1d_sim[expid[i]]['lon'].values[0],
...
CB19_1d_height = find_gridvalue_at_site(
    CB19_DDU_1d_sim[expid[i]]['lat'].values[0],
    CB19_DDU_1d_sim[expid[i]]['lon'].values[0],
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    subset = np.isfinite(CB19_DDU_1d_sim[expid[i]][var_name]) & np.isfinite(CB19_DDU_1d_sim[expid[i]][var_name + '_sim'])
    
    print(np.round(pearsonr(CB19_DDU_1d_sim[expid[i]][var_name][subset], CB19_DDU_1d_sim[expid[i]][var_name + '_sim'][subset], ).statistic ** 2, 3))

print(stats.describe(CB19_1d_oo2q))
# 40.6%

print(find_gridvalue_at_site(
    CB19_DDU_1d_sim[expid[i]]['lat'][0],
    CB19_DDU_1d_sim[expid[i]]['lon'][0],
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.6_CB19/8.3.0.6.0 ' + expid[i] + ' CB19 observed vs. simulated daily ' + var_name + '.png'
    
    fig, ax = plt.subplots(1, 1, figsize=np.array([8.8, 8.8]) / 2.54)
    
    xdata = CB19_DDU_1d_sim[expid[i]][var_name]
    ydata = CB19_DDU_1d_sim[expid[i]][var_name + '_sim']
...
for var_name in ['dD', 'd18O', 'd_xs', 'd_ln', 'q']:
    ...
    output_png = 'figures/8_d-excess/8.3_vapour/8.3.0_obs_vs_sim/8.3.0.6_CB19/8.3.0.6.0 ' + expid[i] + ' CB19 time series of observed vs. simulated daily ' + var_name + '.png'
    
    fig, ax = plt.subplots(1, 1, figsize=np.array([8.8, 6.6]) / 2.54)
    
    xdata = CB19_DDU_1d_sim[expid[i]]['time'].values
    ydata = CB19_DDU_1d_sim[expid[i]][var_name].values
    ydata_sim = CB19_DDU_1d_sim[expid[i]][var_name + '_sim'].values
    ...
    hourly_y = CB19_DDU['1h'][var_name].values
    if (var_name == 'q'):
        hourly_y = hourly_y * 1000
    ax.plot(
        CB19_DDU['1h']['time'].values,
        hourly_y,
...
cplot_ice_cores(CB19_DDU_1d_sim[expid[i]]['lon'][0], CB19_DDU_1d_sim[expid[i]]['lat'][0], ax=ax, s=50, marker='*', edgecolors='b')
...
T63GR15_jan_surf.SLM.sel(lon = CB19_DDU_1d_sim[expid[i]]['lon'][0], lat = CB19_DDU_1d_sim[expid[i]]['lat'][0], method='nearest').values
# DDU is in the Ocean in ECHAM6 T63
