...
lon = hadcm3_extended_regridded_alltime['LIG0.25_extended']['SST']['am'].lon.values
lat = hadcm3_extended_regridded_alltime['LIG0.25_extended']['SST']['am'].lat.values
...
hadcm3_extended_site_values = {}

for iproxy in ['annual_sst', 'summer_sst', 'annual_sat', 'sep_sic',]:
    # iproxy = 'annual_sst'
    print('#-------------------------------- ' + iproxy)
    
    hadcm3_extended_site_values[iproxy] = {}
    
    if (iproxy == 'annual_sst'):
        ivar1 = hadcm3_extended_regridded_alltime['LIG0.25_extended_PI']['SST']['am'].squeeze().values
    elif (iproxy == 'summer_sst'):
        ivar1 = hadcm3_extended_regridded_alltime['LIG0.25_extended_PI']['SST']['sm'].sel(time=3).values
    elif (iproxy == 'annual_sat'):
        ivar1 = hadcm3_extended_regridded_alltime['LIG0.25_extended_PI']['SAT']['am'].squeeze().values
    elif (iproxy == 'sep_sic'):
        ivar1 = hadcm3_extended_regridded_alltime['LIG0.25_extended_PI']['SIC']['mm'].sel(time=9).values * 100
    ...
    for irec in ['EC', 'JH', 'DC', 'MC',]:
        ...
        if (lig_datasets[iproxy][irec] is not None):
            if (lig_twosigma_name[iproxy][irec] is not None):
                hadcm3_extended_site_values[iproxy][irec] = \
            ...
            hadcm3_extended_site_values[iproxy][irec]['sim_lig0.25Sv_extended_pi'] = \
                find_multi_gridvalue_at_site(
                    hadcm3_extended_site_values[iproxy][irec]['Latitude'].values,
                    hadcm3_extended_site_values[iproxy][irec]['Longitude'].values,
                    lat,
                    lon,
                    ivar1,
                )
            
            hadcm3_extended_site_values[iproxy][irec]['sim_rec_lig0.25Sv_extended_pi'] = \
                hadcm3_extended_site_values[iproxy][irec]['sim_lig0.25Sv_extended_pi'] - \
                    hadcm3_extended_site_values[iproxy][irec]['rec_lig_pi']
            
            # print(hadcm3_extended_site_values[iproxy][irec])

with open('scratch/share/from_rahul/data_qingang/hadcm3_extended_site_values.pkl', 'wb') as f:
    pickle.dump(hadcm3_extended_site_values, f)
