...
ds = xr.open_mfdataset(paths=files,use_cftime=True,parallel=True)
siconc = ds.aice.to_dataset().rename(dict(aice='siconc')).isel(time=slice(-1200,None)).compute()
...
lig_sic[model].siconc.values[:] = lig_sic[model].siconc.values * 100
print((lig_sic[model].siconc.values==9.9692100e+36).sum())
print(np.nanmax(lig_sic[model].siconc.values))
...
for model in models:
    print(model)
    print(np.nanmax(lig_sic_regrid[model].siconc[0]))
    print(np.nanmax(pi_sic_regrid[model].siconc[0]))
