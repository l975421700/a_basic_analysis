...
# -----------------------------------------------------------------------------
# region import pre_weighted_rh2m
...
for i in range(len(expid)):
    # i = 0
    with open(exp_odir[i] + expid[i] + '/analysis/echam/' + expid[i] + '.pre_weighted_rh2m.pkl', 'rb') as f:
...
# -----------------------------------------------------------------------------
# region plot am_sm pre_weighted_rh2m

#-------- basic set

lon = pre_weighted_rh2m[expid[0]]['am'].lon
lat = pre_weighted_rh2m[expid[0]]['am'].lat

#-------- plot configuration
output_png = 'figures/6_awi/6.1_echam6/6.1.3_source_var/6.1.3.3_rh2m/' + '6.1.3.3 pi_lig_lgm ' + expid[0] + ' ' + expid[1] + ' ' + expid[2] + ' pre_weighted_rh2m am_sm Antarctica.png'
cbar_label1 = 'Precipitation-weighted open-oceanic source rh2m [$\%$]'

pltlevel = np.arange(73, 85 + 1e-4, 1)
pltticks = np.arange(73, 85 + 1e-4, 1)
...
# -----------------------------------------------------------------------------
# region plot am_sm diff pre_weighted_rh2m
...
#-------- plot configuration
output_png = 'figures/6_awi/6.1_echam6/6.1.3_source_var/6.1.3.3_rh2m/' + '6.1.3.3 pi_lig_lgm ' + expid[0] + ' ' + expid[1] + ' ' + expid[2] + ' pre_weighted_rh2m am_sm diff Antarctica.png'
cbar_label1 = 'Precipitation-weighted open-oceanic source rh2m [$\%$]'
...
for i in np.arange(1, len(expid)):
    ...
    plt_mesh2 = axs[i, 0].pcolormesh(
        lon, lat,
        pre_weighted_rh2m[expid[i]]['am'] - pre_weighted_rh2m[expid[0]]['am'],
    ...
    for iseason in range(len(seasons)):
        ...
        axs[i, iseason + 1].pcolormesh(
            lon, lat,
            pre_weighted_rh2m[expid[i]]['sm'].sel(season=seasons[iseason]) - \
                pre_weighted_rh2m[expid[0]]['sm'].sel(season=seasons[iseason]),
            norm=pltnorm2, cmap=pltcmp2,transform=ccrs.PlateCarree(),)
