...
# (era5_mon_sl_20_gph.z[0, :, :] == era5_mon_sl_20_gph.z[1, :, :]).all()
...
# np.max(abs(
#     era5_mon_sl_20_gph.z[0, :, :].squeeze().values - \
#     (mpcalc.height_to_geopotential(era5_topograph.magnitude * units.m)).magnitude))

fig, ax = framework_plot1("global", figsize=np.array([8.8*2, 10.5]) / 2.54)

pltlevel = np.arange(0, 6001, 50)
pltticks = np.arange(0, 6001, 1000)

plt_cmp = ax.pcolormesh(
    era5_mon_sl_20_gph.longitude.values,
    era5_mon_sl_20_gph.latitude.values,
...
cbar = fig.colorbar(
    ...
    fraction=0.09, shrink=0.6, aspect=25, anchor=(0.5, -2),
...
pltlevel_nh = np.arange(0, 200.01, 0.5)
pltticks_nh = np.arange(0, 200.01, 25)

pltlevel_sh = np.arange(0, 200.01, 0.5)
pltticks_sh = np.arange(0, 200.01, 25)
...
# =============================================================================
# region plot NH monthly precipitation in era5
fig, ax = hemisphere_plot(southextent=45, sb_length=2000, sb_barheight=200,)

plt_cmp = ax.pcolormesh(
    era5_mon_sl_79_21_pre.longitude.values,
    era5_mon_sl_79_21_pre.latitude.values[0:241],
    pre_mon_sum.sel(month=1)[0:241, ],
    norm=BoundaryNorm(pltlevel_nh, ncolors=len(pltlevel_nh), clip=False),
    cmap=cm.get_cmap('Blues', len(pltlevel_nh)), rasterized=True,
    transform=ccrs.PlateCarree(),
)
cbar = fig.colorbar(
...
# =============================================================================
# region animate NH monthly precipitation in era5
...
for i in range(12):  # range(2):  #
    ...
    plt_cmp = ax.pcolormesh(
        era5_mon_sl_79_21_pre.longitude.values,
        era5_mon_sl_79_21_pre.latitude.values[0:241],
        pre_mon_sum.sel(month=(i+1))[0:241, ],
        norm=BoundaryNorm(pltlevel_nh, ncolors=len(pltlevel_nh), clip=False),
        cmap=cm.get_cmap('Blues', len(pltlevel_nh)), rasterized=True,
        transform=ccrs.PlateCarree(),
    )
...
ani = animation.ArtistAnimation(fig, ims, interval=500, blit=True)
ani.save(
    'figures/02_era5/02_01_era5_pre/02.01.02 NH monthly precipitation in ERA5.mp4',
    progress_callback=lambda i, n: print(f'Saving frame {i} of {n}'),)
...
# =============================================================================
# region plot SH monthly precipitation in era5
fig, ax = hemisphere_plot(northextent=-45, sb_length=2000, sb_barheight=200,)

plt_cmp = ax.pcolormesh(
    era5_mon_sl_79_21_pre.longitude.values,
    era5_mon_sl_79_21_pre.latitude.values[480:],
    pre_mon_sum.sel(month=1)[480:, ],
...
# =============================================================================
# region animate SH monthly precipitation in era5
...
ani = animation.ArtistAnimation(fig, ims, interval=500, blit=True)
ani.save(
    'figures/02_era5/02_01_era5_pre/02.01.03 SH monthly precipitation in ERA5.mp4',
    progress_callback=lambda i, n: print(f'Saving frame {i} of {n}'),)
...
pltlevel_nh = np.arange(0, 2000.01, 5)
pltticks_nh = np.arange(0, 2000.01, 400)

pltlevel_sh = np.arange(0, 2000.01, 5)
pltticks_sh = np.arange(0, 2000.01, 400)
...
# =============================================================================
# region plot NH annual precipitation in era5
fig, ax = hemisphere_plot(southextent=45, sb_length=2000, sb_barheight=200,)

plt_cmp = ax.pcolormesh(
    era5_mon_sl_79_21_pre.longitude.values,
    era5_mon_sl_79_21_pre.latitude.values[0:241],
    pre_ann_sum.sel(year=1979)[0:241, ],
    norm=BoundaryNorm(pltlevel_nh, ncolors=len(pltlevel_nh), clip=False),
    cmap=cm.get_cmap('Blues', len(pltlevel_nh)), rasterized=True,
    transform=ccrs.PlateCarree(),
)
...
# =============================================================================
# region animate NH annual precipitation in era5
fig, ax = hemisphere_plot(southextent=45, sb_length=2000, sb_barheight=200,)
ims = []
...
for i in range(len(pre_ann_sum.year) - 1):  # range(2):  #
    ...
    plt_cmp = ax.pcolormesh(
        era5_mon_sl_79_21_pre.longitude.values,
        era5_mon_sl_79_21_pre.latitude.values[0:241],
        pre_ann_sum.sel(year=(i+1979))[0:241, ],
        norm=BoundaryNorm(pltlevel_nh, ncolors=len(pltlevel_nh), clip=False),
        cmap=cm.get_cmap('Blues', len(pltlevel_nh)), rasterized=True,
        transform=ccrs.PlateCarree(),
    )
    ...
    textinfo = ax.text(
        -0.1, 1, pre_ann_sum.year[i].values, backgroundcolor='white',
        transform=ax.transAxes, fontweight='bold', ha='center', va='center',
    )
...
# =============================================================================
# region plot SH annual precipitation in era5
fig, ax = hemisphere_plot(northextent=-45, sb_length=2000, sb_barheight=200,)

plt_cmp = ax.pcolormesh(
    era5_mon_sl_79_21_pre.longitude.values,
    era5_mon_sl_79_21_pre.latitude.values[480:],
    pre_ann_sum.sel(year=1979)[480:, ],
    norm=BoundaryNorm(pltlevel_sh, ncolors=len(pltlevel_sh), clip=False),
    cmap=cm.get_cmap('Blues', len(pltlevel_sh)), rasterized=True,
    transform=ccrs.PlateCarree(),
)
...
for i in range(len(pre_ann_sum.year) - 1):  # range(2):  #
    ...
    plt_cmp = ax.pcolormesh(
        era5_mon_sl_79_21_pre.longitude.values,
        era5_mon_sl_79_21_pre.latitude.values[480:],
        pre_ann_sum.sel(year=(i+1979))[480:,],
        norm=BoundaryNorm(pltlevel_sh, ncolors=len(pltlevel_sh), clip=False),
        cmap=cm.get_cmap('Blues', len(pltlevel_sh)), rasterized=True,
        transform=ccrs.PlateCarree(),
    )
...
ani = animation.ArtistAnimation(fig, ims, interval=250, blit=True)
ani.save(
    'figures/02_era5/02_01_era5_pre/02.01.05 SH Annual precipitation in ERA5.mp4',
    progress_callback=lambda i, n: print(f'Saving frame {i} of {n}'),)
